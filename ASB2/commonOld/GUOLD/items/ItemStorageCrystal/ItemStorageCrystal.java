...
public class ItemStorageCrystal extends ItemBase {
    ...
    public void getSubItems(Item id, CreativeTabs tab, List list) {
        ...
        for (ItemStack stack : GUOLD.FluidRegistry.STORAGE_CRYSTAL_LIST) {
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitx, float hity, float hitz) {
            ...
            if (stack.getFluid().getBlock() != null) {
                ...
                if (!player.capabilities.isCreativeMode) {
                    ...
                    if (UtilInventory.consumeItemStack(player.inventory, itemStack, 1) && UtilInventory.addItemStackToInventory(player.inventory, new ItemStack(ItemRegistry.ItemStorageCrystal, 1, 0), true)) {
                        ...
                        return UtilBlock.placeBlockInAir(world, coords[0], coords[1], coords[2], stack.getFluid().getBlock(), 0);
                    }
                }
                else {
        ...
        // if(fluid != null) {
        //
        // ItemStack filled = FluidContainerRegistry.fillFluidContainer(new
        // FluidStack(fluid, 1000), new
        // ItemStack(ItemRegistry.ItemStorageCrystal, 1, 1));
        //
        // if(filled != null) {
        //
        // if(!player.capabilities.isCreativeMode) {
        //
        // if(UtilInventory.addItemStackToInventory(player.inventory, filled,
        // true) && UtilInventory.consumeItemStack(player.inventory, new
        // ItemStack(ItemRegistry.ItemStorageCrystal, 1, 0), 1)) {
        //
