...
public class CanvasRenderer implements ISimpleBlockRenderingHandler  {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        ...
        if(tile != null && tile instanceof IColorable) {
            ...
            UtilRender.renderFakeBlock(renderer, block, x, y, z, ((BlockCanvas)BlockRegistry.BlockCanvas).inner, 1, 1, 1, 1, block.getMixedBrightnessForBlock(world, x, y, z));
        }
        else {
            
            renderer.setRenderBounds(0.001, 0.001, 0.001, .999, .999, .999);
            UtilRender.renderFakeBlock(renderer, block, x, y, z, block.getIcon(0, 0), 1, 1, 1, 1, block.getMixedBrightnessForBlock(world, x, y, z));
            
            renderer.setRenderBounds(0, 0, 0, 1, 1, 1);
            UtilRender.renderFakeBlock(renderer, block, x, y, z, ((BlockCanvas)BlockRegistry.BlockCanvas).inner, 1F, 1, 1, 1, block.getMixedBrightnessForBlock(world, x, y, z));
        }
        return true;
    }
